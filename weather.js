async function getWeather(cityName) {
  ...
  const options = {
    method: 'GET',
    headers: {
      'X-RapidAPI-Key': '44a6ee714amsh87c7c8592f2b4c5p12dca3jsncf6d8cab1464',
  ...
  await fetch(url, options)
    ...
    .then(data => {
      console.log(data);
      const cityDescriptionDiv = document.getElementById("city-description")

      cityDescriptionDiv.innerHTML = `Showing the weather of ${cityName}, ${data.location.country}`;
      cityDescriptionDiv.setAttribute("class", "DescDiv")
      getCurrentWeather(data);
      getForcastWeather(data)
...
function getForcastWeather(weatherData) {
